    ...
    if (#interfaces_a > 0) then
        if (#interfaces_a > 1) then
            for i = 1, #interfaces_a do
                if (check_network_connected(routes_text, interfaces_a[i])) then
                    if (i ~= #interfaces_a) then
                        result = result ..
                                     "${alignr}${offset -32}                                                           │\n"
                        result = result .. "${alignr}${offset -32}[ ${addr " .. interfaces_a[i] .. "} ] " ..
                                     interfaces_a[i] .. " ─┤\n"
                        result = result .. "${alignr}${offset -32}[ ${downspeed " .. interfaces_a[i] ..
                                     "} k/s ] download ─┤    │\n"
                        result = result .. "${alignr}${offset -32}[ ${downspeedgraph " .. interfaces_a[i] ..
                                     " 12,120 136311 11B014} ] ─┘    │    │\n"
                        result = result .. "${alignr}${offset -32}[ ${upspeed " .. interfaces_a[i] ..
                                     "} k/s ]   upload ─┘    │\n"
                        result = result .. "${alignr}${offset -32}[ ${upspeedgraph " .. interfaces_a[i] ..
                        ...
                        result = result .. "${alignr}${offset -32}[ ${upspeed " .. interfaces_a[i] ..
                                     "} k/s ]   upload ─┘     \n"
                        result = result .. "${alignr}${offset -62}[ ${upspeedgraph " .. interfaces_a[i] ..
                                     " 12,120 BA0B0B FC0707} ] ─┘     \n"
                    end
                end
            end
            ...
            if (check_network_connected(routes_text, interfaces_a[1])) then
                result = result ..
                             "${alignr}${offset -32}                                                           │\n"
                result = result .. "${alignr}${offset -32}[ ${addr " .. interfaces_a[1] .. "} ] " .. interfaces_a[1] ..
                             " ─┘\n"
                result = result .. "${alignr}${offset -32}[ ${downspeed " .. interfaces_a[1] ..
                             "} k/s ] download ─┤     \n"
                result = result .. "${alignr}${offset -32}[ ${downspeedgraph " .. interfaces_a[1] ..
                             " 12,120 136311 11B014} ] ─┘    │     \n"
                result = result .. "${alignr}${offset -32}[ ${upspeed " .. interfaces_a[1] ..
                             "} k/s ]   upload ─┘     \n"
                result = result .. "${alignr}${offset -62}[ ${upspeedgraph " .. interfaces_a[1] ..
                             " 12,120 BA0B0B FC0707} ] ─┘     \n"
            else
                result = "${alignr}${offset -32} No Network Available ─┘"
            end
        end
    else
        result = "${alignr}${offset -32} No Network Available ─┘"
